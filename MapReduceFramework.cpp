...
void mapPhase(ThreadContext* context)
{
    ...
    while (oldVal < (unsigned int) jc->inputVector->size())
    {
        ...
        float perc_val = ((float)(jc->mapProgressCounter) / (float)(jc->inputVector->size())) * 100 ;
        if (perc_val > 100.0) {
            perc_val = 100.0;
        }
        jc->state.percentage = perc_val;
...
void shufflePhase(ThreadContext *context) {
    ...
    while(true) {
        ...
        for (int i = 0; i < jc->MULTI_THREAD_NUM; i++) {
            if (jc->threadContextsVec[i].mapOutputVector->empty()) {
                continue;
            }
            else {
                cur_pair = jc->threadContextsVec[i].mapOutputVector->back();
                if (!max_pair.first || sortKeys(max_pair, cur_pair)) {
                    max_pair = cur_pair;
                }
            }
        }
        ...
        for (int i = 0; i < jc->MULTI_THREAD_NUM; i++) {
            ...
            while (!(*jc->threadContextsVec[i].mapOutputVector->back().first < *max_pair.first) &&
                ...
                auto pair = jc->threadContextsVec[i].mapOutputVector->back();
                jc->threadContextsVec[i].mapOutputVector->pop_back();
                temp_vec->push_back(pair);

                mutexLock(&jc->stateMutex);
                float perc_val = (float)(jc->shuffleProgressCounter) / (float)(jc->numAllIntermediaryPairs) * 100;
                if (perc_val > 100.0) {
                    perc_val = 100.0;
                }
                jc->state.percentage = perc_val;
        ...
        if (jc->numIntermediaryElements == 0) {
            break;
        }
//        delete temp_vec;
...
void reducePhase(ThreadContext *context) {
    ...
    while (ind < jc->shuffleVec.size()) {
        ...
        mutexLock(&jc->stateMutex);
        float perc_val = ((float) (jc->reduceProgressCounter) / (float) (jc->shuffleVec.size())) * 100;
        if (perc_val > 100.0) {
            perc_val = 100.0;
        }
        jc->state.percentage = perc_val;
...
void initMutexes(jobContext *jc) {
    ...
    if(pthread_mutex_init(&jc->waitForJobMutex, nullptr) != 0)
    {
        std::cerr << SYS_ERROR << MUTEX_CREATE_FAIL << std::endl;
        exit(FAILURE);
    }
...
//        mutexLock()
    ...
    mutexUnlock(&jc->waitForJobMutex);
}
...
void closeJobHandle(JobHandle job) {
    ...
    if (pthread_mutex_destroy(&currJob->waitForJobMutex) != 0) {
        std::cerr << SYS_ERROR << MUTEX_DESTROY_FAIL << std::endl;
        exit(FAILURE);
    }

//    for (int i = 0; i < currJob->MULTI_THREAD_NUM; i++) {
//        delete currJob->threadContextsVec->mapOutputVector;
//        delete currJob->threadContextsVec->barrier;
//    }
